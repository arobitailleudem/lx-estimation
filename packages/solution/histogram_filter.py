    ...
    for i in range(belief.shape[0]):
        for j in range(belief.shape[1]):
            ...
            if belief[i, j] > 0:
                ...
                i_new = int(np.clip(np.floor((d_t[i, j] - grid_spec["d_min"]) / grid_spec["delta_d"]), 0, belief.shape[0] - 1))
                j_new = int(np.clip(np.floor((phi_t[i, j] - grid_spec["phi_min"]) / grid_spec["delta_phi"]), 0, belief.shape[1] - 1))
                p_belief[i_new, j_new] += belief[i, j]



                p_belief[i_new, j_new] = p_belief[i_new, j_new] + belief[i, j]
...
def generate_measurement_likelihood(segments, road_spec, grid_spec):
    ...
    for segment in segments:
        ...
        i = int(np.clip(np.floor((d_i - grid_spec["d_min"]) / grid_spec["delta_d"]), 0, measurement_likelihood.shape[0] - 1))
        j = int(np.clip(np.floor((phi_i - grid_spec["phi_min"]) / grid_spec["delta_phi"]), 0, measurement_likelihood.shape[1] - 1))
